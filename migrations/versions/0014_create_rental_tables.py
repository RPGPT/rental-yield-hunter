...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("lifetime_rent", sa.Boolean(), nullable=True),
        ...
        sa.Column("inactive_since", sa.TIMESTAMP(), nullable=True),
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["listing_id"], ["rental_listings.id"]),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_table(
        "rental_raw_data",
...
def downgrade() -> None:
    op.drop_table("rental_raw_data")
    op.drop_table("rental_listing_price_history")
